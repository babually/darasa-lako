...
type ResourcePayload = Prisma.ResourceGetPayload<{
...
interface ResourcesState {
	...
	addResource: (
		resource: Omit<Prisma.ResourceCreateInput, "uploader">,
	) => Promise<void>;
	updateResource: (
		resourceId: string,
		data: Omit<Prisma.ResourceUpdateInput, "uploader">,
	) => Promise<void>;
...
export function ResourcesProvider({ children }: { children: React.ReactNode }) {
	...
	const loadResources = useCallback(async () => {
		try {
			const data = await getClient().resource.findMany({
				include: { uploader: true },
			});
			setResources(data);
		} catch (error) {
	...
	useEffect(() => {
		...
		unsubscribeResource.current = getClient().resource.subscribe(
	...
	const addResource = useCallback(
		async (resource: Omit<Prisma.ResourceCreateInput, "uploader">) => {
			try {
