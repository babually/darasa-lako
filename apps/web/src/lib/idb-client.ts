...
async function initializeClient() {
	if (!browser)
		throw new Error("PrismaIDBClient can only be initialized in the browser");
	if (!initPromise) {
		initPromise = PrismaIDBClient.createClient().then(
			(c: PrismaIDBClient) => (client = c),
		);
	}
	await initPromise;
	return client;
...
function getClient(): PrismaIDBClient {
	if (!client)
		throw new Error(
			"PrismaIDBClient not initialized. Call initializeClient() first.",
		);
	return client;
}
